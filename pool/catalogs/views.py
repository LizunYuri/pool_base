    ...
    data = [
        {
            ...
            'supplier' : {
                ...
                'url' : instance.supplier.url if instance.supplier else None,
                },
...
def export_finished(request):
    return export_data(request, FinishingMaterialsModel)
...
@csrf_exempt
def update_finished(request):
    if request.method == 'POST':
        data = json.loads(request.body)
        updated_finished = [update_item(item, FinishingMaterialsModel) for item in data]
        return JsonResponse(updated_finished, safe=False)
